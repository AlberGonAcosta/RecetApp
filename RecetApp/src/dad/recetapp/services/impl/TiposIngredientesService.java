...
public class TiposIngredientesService implements ITiposIngredientesService {
	...
	public void crearTipoIngrediente(TipoIngredienteItem tipo) throws ServiceException {
		try{
			if(tipo == null || tipo.getNombre() == null)
				throw new ServiceException("Error al crear el tipo de ingrediente: El tipo no puede ser nulo.");
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn.prepareStatement("INSERT INTO tipos_ingredientes (nombre) VALUES (?)",PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public void modificarTipoIngrediente(TipoIngredienteItem tipo) throws ServiceException {
		try {
			...
			throw new ServiceException("Error al modificar el tipo de ingrediente: El tipo no puede ser nulo.");
		}
	}
	...
	public void eliminarTipoIngrediente(Long id) throws ServiceException {
		try {
			...
			throw new ServiceException("Error al eliminar el tipo de ingrediente '"+id+"': "+ e.getMessage());
		} catch (NullPointerException e){
			throw new ServiceException("Error al eliminar el tipo de ingrediente: El tipo no puede ser nulo.");
		}
	}
	...
	public List<TipoIngredienteItem> listarTiposIngredientes() throws ServiceException {
		...
		try {
			Connection conn = BaseDatos.getConnection();
			PreparedStatement statement = conn.prepareStatement("SELECT * FROM tipos_ingredientes ORDER BY nombre");
			...
			throw new ServiceException("Error al recuperar los tipos de ingrediente", e);
		}
		return tipos;
	}
	...
	public TipoIngredienteItem obtenerTiposIngredientes(Long id) throws ServiceException {
		if(id == null)
			throw new ServiceException("Error al recuperar el tipo de ingrediente: Debe especificar un id válido.");
